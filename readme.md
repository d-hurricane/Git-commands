...
git cherry-pick --abort          # прервать конфликтный перенос коммитов, отменить внесенные изменения
git cherry-pick --quit           # прервать конфликтный перенос коммитов, не отменять уже сделанные частичные изменения
...
git log -p -22             # показать последние 22 коммита и внесенную ими разницу (diff) на уровне строк
git log --oneline          # показать коммиты, каждый на отдельной строке с сокращенным идентификатором и заголовком
...
git log --grep fix         # показать коммиты в текущей ветке, в описании которых есть буквосочетание fix (регистрозависимо, только коммиты текущей ветки)
git log --grep fix branch_99 -i # показать коммиты в ветке branch_99, в описании которых есть буквосочетание fix (регистроНЕзависимо, только коммиты текущей ветки)
...
git log master..branch_99 --boundary  # показать коммиты из ветки branch_99, которые не влиты в master, начиная с их расхождения (коммит расхождения будет показан)
git log branch_99..master  # показать коммиты из ветки master, которые не влиты в branch_99
git log branch_99 ^master  # аналогичная операция - коммиты из master без коммитов branch_99
git log master...branch_99 # показать коммиты из указанных веток, начиная с их расхождения, без общих коммитов
```
...
git merge hotfix --no-commit # влить в текущую ветку данные из ветки hotfix, изменив файлы в рабочей дирректории, но не делая при этом коммит в репозиторий
git merge hotfix --squash  # применить изменения из ветки hotfix для текущей ветки без слияния
...
git merge --abort # то же, что и строкой выше
```
...
git rebase --quit     # прервать конфликтный rebase, не возвращая при этом рабочую директорию и индекс к состоянию до начала rebase
...
``` bash
git config pretty.my format:"%h - %an, %ar : %s"  # создать алиас "my" для использования командой log: git log --pretty=my
git config format.pretty my     # использовать нового алиаса для форматирования по умолчанию при выводе лога
git config log.date short       # использовать короткую запись даты по умолчанию при выводе лога
git config merge.conflictStyle diff3 # показывать в конфликтном файле при слиянии сравнение содержимого веток, а также то, чтобы было в месте конфликта в момент разделения веток.
git config merge.ff false       # отключить быструю перемотку при слиянии для всех веток
git config branch.master.mergeoptions '--no-ff' # настройки слияния по умолчанию для ветки master, в данном случае запрет на слияние перемоткой
```
